...
export async function walmartFetch(url: string): Promise<FetchResult> {
  const normalizedUrl = normalizeWalmartUrl(url)
...
function normalizeWalmartUrl(url: string): string {
  try {
    const parsed = new URL(url)
    parsed.searchParams.delete('rd')
    parsed.searchParams.delete('affin')
    parsed.searchParams.delete('affsite')
    parsed.searchParams.delete('veh')
...
export function walmartParse(html: string): ParsedData {
  ...
  const totalReviews = totalReviewsText ? parseInt(totalReviewsText, 10) : null

  const reviewElements = $('[data-testid="customer-review"], .review-item, .bg-white.border-bottom')
  reviewElements.each((index, el) => {
    ...
    const title = $el.find('[data-testid="review-title"], .review-title, h3').first().text().trim()

    const dateText = $el.find('[itemprop="datePublished"]').attr('content') ||
      $el.find('.review-date').text().trim()
    const date = dateText || null

    const author = $el.find('[itemprop="author"]').text().trim() ||
      $el.find('.reviewer-name').text().trim() ||
      'Anonymous'

    const verified = $el.find('.verified-purchase').length > 0 ||
      $el.text().toLowerCase().includes('verified purchaser')

    const snippet = $el.find('[itemprop="reviewBody"], .review-text, p').text().trim()

    const helpfulText = $el.find('.helpful-count, [data-testid="helpful-count"]').text() || ''
  ...
  return {
    ...
    averageRating,
    totalReviews,
    category: walmartExtractCategory(html, productName),
    metadata: { parseMethod: 'walmart-specific' },
  }
}

export function walmartExtractCategory(html: string, productName: string | null): string {
